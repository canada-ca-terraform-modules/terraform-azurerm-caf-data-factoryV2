...
resource "azurerm_data_factory" "df" {
  name                            = try(var.data_factory.name, local.data-factory-name)
  ...
  identity {
    ...
    # Explicit identity_ids override takes priority (existing UAMIs not created by this module);
    # otherwise fall back to the UAMI this module creates when the type includes UserAssigned.
    identity_ids = try(var.data_factory.identity.identity_ids, null) != null ? (
      var.data_factory.identity.identity_ids
      ) : (
      strcontains(var.data_factory.identity.type, "UserAssigned") ? [azurerm_user_assigned_identity.ui[0].id] : null
    )
  }
  ...
  dynamic "github_configuration" {
    ...
    content {
      ...
      git_url            = try(var.data_factory.github_configuration.git_url, null)
...
resource "azurerm_key_vault_secret" "secret" {
  name            = try(var.data_factory.secret_name, "${local.data-factory-name}-secret")
  value           = random_string.password.result
  key_vault_id    = var.key_vault.id
  content_type    = try(var.data_factory.secret_content_type, null)
  not_before_date = try(var.data_factory.secret_not_before_date, null)
  expiration_date = try(var.data_factory.secret_expiration_date, null)
  tags            = try(var.data_factory.secret_tags, null)
}

resource "azurerm_data_factory_linked_service_key_vault" "lskv" {
  name                     = try(var.data_factory.key_vault_linked_service.name, "${local.data-factory-name}-kv-linked-service")
  data_factory_id          = azurerm_data_factory.df.id
  key_vault_id             = var.key_vault.id
  description              = try(var.data_factory.key_vault_linked_service.description, null)
  integration_runtime_name = try(var.data_factory.key_vault_linked_service.integration_runtime_name, null)
  annotations              = try(var.data_factory.key_vault_linked_service.annotations, null)
  parameters               = try(var.data_factory.key_vault_linked_service.parameters, null)
  additional_properties    = try(var.data_factory.key_vault_linked_service.additional_properties, null)
}

resource "azurerm_data_factory_credential_service_principal" "spn" {
  count                = try(var.data_factory.service_principal, null) == null ? 0 : 1
  name                 = try(var.data_factory.service_principal.name, "${local.data-factory-name}-spn")
  ...
  annotations = try(var.data_factory.service_principal.annotations, null)
}
...
resource "azurerm_user_assigned_identity" "ui" {
  ...
  resource_group_name = local.resource_group_name
  isolation_scope     = try(var.data_factory.user_assigned_identity.isolation_scope, null)
  tags                = try(var.data_factory.user_assigned_identity.tags, var.tags)
...
resource "azurerm_data_factory_credential_user_managed_identity" "mi" {
  ...
  annotations = try(var.data_factory.user_managed_identity.annotations, null)
}
